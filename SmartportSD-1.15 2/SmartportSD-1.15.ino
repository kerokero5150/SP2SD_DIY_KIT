...
// Current values
#define WRPROT          19     // WRPROT is on pin 19
#define ACT_LED         18     // Activity LED is on pin 18
#define EJECT_SWITCH    17     // The Eject Switch is on pin 17

// The following are values used for direct port manipulation through bits
// See https://docs.arduino.cc/hacking/software/PortManipulation
#define DDRC_ACK_INPUT  0xDF   // Bitmask to set ACK to input through the port C data direction register
#define DDRC_ACK_OUTPUT 0xFF   // Bitmask to set ACK to output through the port C data direction register
#define PORTC_ACK_BIT   5      // Location of WRPROT in the port C data register
#define PINC_ACK_SET    0x20   // Value to check the port C input pins register against when checking if WRPROT is set

/*
// Future values
#define ACT_LED         17     // Activity LED is on pin 17
#define WRPROT          16     // WRPROT is on pin 16
#define EJECT_SWITCH    15     // The Eject Switch is on pin 15

// The following are values used for direct port manipulation through bits
// See https://docs.arduino.cc/hacking/software/PortManipulation
#define DDRC_ACK_INPUT  0xFB   // Bitmask to set ACK to input through the port C data direction register
#define DDRC_ACK_OUTPUT 0xFF   // Bitmask to set ACK to output through the port C data direction register
#define PORTC_ACK_BIT   2      // Location of WRPROT in the port C data register
#define PINC_ACK_SET    0x04   // Value to check the port C input pins register against when checking if WRPROT is set
*/
...
void loop() {
  ...
  while (1) {
    ...
    if (digitalRead(EJECT_SWITCH) == HIGH) rotate_boot();


    noid = 0;     //reset noid flag
    DDRC = DDRC_ACK_INPUT;  //set ack (wrprot) to input to avoid clashing with other devices when sp bus is not enabled
    ...
    switch (phases) {
      ...
      case 0x0f:
        ...
        noInterrupts();
        DDRC = DDRC_ACK_OUTPUT;  //set ack to output, sp bus is enabled
        ...
        if (packet_buffer[14] != 0x85)  // if its an init pkt, then assume its for us and continue on
        {
          ...
          if (noid == NUM_PARTITIONS)  //not one of our id's
          {
            ...
            Serial.print(F("\r\nNot our ID!"));
            //printf_P(PSTR("\r\nnot ours\r\n") );
            DDRC = DDRC_ACK_INPUT;           //set ack to input, so lets not interfere
            PORTC &= ~(_BV(PORTC_ACK_BIT));  //set ack low, for next time its an output
            while (PINC & PINC_ACK_SET)
        ...
        //Serial.print(F("\r\nBW"));
        PORTC &= ~(_BV(PORTC_ACK_BIT));  //set ack low
        while (PIND & 0x04)
